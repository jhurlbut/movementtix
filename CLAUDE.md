...
`src/movementtix/odyssey.py` (`python -m movementtix.odyssey`) watches **two venues** for The Odyssey in IMAX 70mm: evening shows (5:00–9:59pm start; `WATCH_MAX_HOUR`) with two adjacent available seats in the **back 2/3 of the house** — cutoff computed per seat map from the rows present (row E+ at Metreon's 13-row house, D+ at Hacienda's 9-row house), with `WATCH_MIN_ROW` as an extra floor.

- **AMC Metreon 16 (SF):** AMC's Queue-it/Cloudflare blocks its JS bundles but the Next.js RSC payload in the HTML carries showtimes + seat maps, so we parse `self.__next_f.push` blobs directly with plain headless Chromium (no relay/proxy needed). Health canary hits the theater's live showtimes page first — a scan is only trusted if the canary parses real showtimes.
- **Regal Hacienda Crossings (Dublin):** regmovies.com JSON APIs (`GetTheatreFilmDays`, `getShowtimes`, `GetSeatPlan`) 403 outside a Turnstile-cleared browser. Stock headless Chromium never clears; real google-chrome headed under xvfb clears in seconds. Each scan launches a short-lived headed Chrome (fresh process, `~/.regal-chrome` profile persists cf_clearance; no long-lived relay to degrade) and drives the APIs via in-page `fetch()`. `GetSeatPlan` is burst-rate-limited (~20/window → 403s, then 401s): seat checks are budgeted (`REGAL_SEAT_BUDGET`, default 16) — every show in the next 7 days each cycle, plus a rotating slice of the far tail; on 2 consecutive hard failures the cycle's remaining seat checks abort. Per-venue health + dedupe keys (`amc:`/`regal:` prefixes) in `state.db` kv.
